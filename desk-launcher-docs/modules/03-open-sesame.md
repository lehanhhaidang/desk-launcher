...
| `rust/src/backup.rs` | `export_data(ExportOptions) -> Result<ModuleExport, BackupError>` — snapshots the Open Sesame SQLite DB via `dbsnap` and reads the GitHub OAuth token from the keyring; when `include_heavy` is set, also bundles the local git mirror folders (`~/.open-sesame/mirrors/<id>/`). `import_data(ModuleImport, ImportMode) -> Result<(), BackupError>` — restores the DB via `dbsnap::restore_db` and writes the token back to the keyring. Consumed by the launcher host backup orchestrator. |
