...
| 🗣️ **Virtual Comtor** | Phiên dịch real-time Nhật ↔ Việt cho meeting (Soniox + OpenAI), transcript + summary + xlsx export. | `rusqlite` + `keyring` (Windows Credential Manager) |
...
# Tải bundled binaries cho Video Downloader (xem khối Binaries phía dưới)
...
Tauri externalBin yêu cầu file với suffix target triple. Tải về:

```powershell
$bin = "apps\launcher\src-tauri\binaries"
$target = "x86_64-pc-windows-msvc"

# yt-dlp (~18 MB)
Invoke-WebRequest "https://github.com/yt-dlp/yt-dlp/releases/latest/download/yt-dlp.exe" `
  -OutFile "$bin\yt-dlp-$target.exe"

# ffmpeg essentials (~100 MB)
$tmp = "$env:TEMP\ffmpeg-essentials.zip"
Invoke-WebRequest "https://www.gyan.dev/ffmpeg/builds/ffmpeg-release-essentials.zip" -OutFile $tmp
Expand-Archive $tmp -DestinationPath "$env:TEMP\ffmpeg-extract" -Force
$ff = Get-ChildItem "$env:TEMP\ffmpeg-extract" -Recurse -Filter ffmpeg.exe | Select-Object -First 1
Copy-Item $ff.FullName "$bin\ffmpeg-$target.exe"
Remove-Item $tmp, "$env:TEMP\ffmpeg-extract" -Recurse -Force
```
