//! Video Downloader Tauri plugin.
//!
//! Spawns bundled `yt-dlp.exe` (and indirectly `ffmpeg.exe`) as Tauri sidecars.
//!
//! Sidecar binaries live in `apps/launcher/src-tauri/binaries/` and are declared
//! in `tauri.conf.json` under `bundle.externalBin`. Tauri picks the right
//! target-triple-suffixed exe at runtime.
//!
//! Commands:
//!   - `video_info(url)`              — yt-dlp --dump-single-json
//!   - `video_download_start(args)`   — spawn yt-dlp; returns task_id immediately
//!   - `video_download_cancel(task_id)`
//!   - `video_download_read(task_id)` — read finished file bytes
//!   - `video_download_cleanup(task_id)` — delete finished file
//!
//! Progress streaming: emits `video-progress` events with `DownloadProgress`
//! payload while the download runs. Frontend listens via
...
const EVENT_PROGRESS: &str = "video-progress";
...
#[tauri::command]
async fn video_info(app: AppHandle, url: String) -> Result<VideoInfo, String> {
    let cmd = app
        .shell()
        .sidecar(SIDECAR_NAME)
        .map_err(|e| format!("yt-dlp sidecar unavailable: {e}. Đảm bảo bundled binaries ở src-tauri/binaries/"))?
...
fn parse_info(j: &serde_json::Value) -> VideoInfo {
    ...
    video_formats.sort_by(|a, b| b.tbr.unwrap_or(0.0).partial_cmp(&a.tbr.unwrap_or(0.0)).unwrap_or(std::cmp::Ordering::Equal));
    audio_formats.sort_by(|a, b| b.abr.unwrap_or(0.0).partial_cmp(&a.abr.unwrap_or(0.0)).unwrap_or(std::cmp::Ordering::Equal));
    ...
    VideoInfo {
        ...
        channel: j.get("channel").or_else(|| j.get("uploader")).and_then(|v| v.as_str()).map(String::from),
...
#[tauri::command]
async fn video_download_start(
    app: AppHandle,
    args: DownloadArgs,
) -> Result<String, String> {
    let task_id = uuid::Uuid::new_v4().to_string();
    let download_dir = launcher_paths::module_data_dir("video-downloader")
        .map_err(|e| format!("module dir: {e}"))?
        .join("downloads");
    std::fs::create_dir_all(&download_dir).map_err(|e| format!("mkdir: {e}"))?;
    ...
    // yt-dlp args. `--newline` makes it emit progress one-per-line.
    // `--progress-template` controls what each line contains; we parse it.
    ...
    // Background task: stream progress events, emit completed/failed.
            ...
            match event {
                CommandEvent::Stdout(line) => {
                    ...
                    if let Some((pct, downloaded, total, speed, eta)) = parse_progress(&text) {
                        ...
                        let _ = app_bg.emit(EVENT_PROGRESS, DownloadProgress {
                ...
                CommandEvent::Terminated(payload) => {
                    ...
                    let cancelled = {
                        let state = app_bg.state::<VideoState>();
                    ...
                    let (status, filepath, filename, error) = if cancelled {
                        ...
                        match find_output_file(&download_dir_bg, &task_id_bg, &output_type_bg) {
                            Some(p) => {
                                ...
                                ("completed".into(), Some(p.to_string_lossy().to_string()), fname, None)
                            }
                            None => ("failed".into(), None, None, Some("output file not found".into())),
                        }
                    } else {
                        ("failed".into(), None, None, Some(format!("yt-dlp exit code {:?}", payload.code)))
                    };
...
fn parse_progress(line: &str) -> Option<(f32, i64, i64, String, String)> {
    // Format: [progress]<downloaded>/<total>|<speed>|<eta>
...
fn find_output_file(dir: &std::path::Path, task_id: &str, output_type: &str) -> Option<PathBuf> {
    let preferred_ext = if output_type == "mp3" { "mp3" } else { "mp4" };
    let preferred = dir.join(format!("{task_id}.{preferred_ext}"));
    if preferred.exists() {
        return Some(preferred);
    }
    // Fallback: any file starting with task_id.
...
#[tauri::command]
fn video_download_cancel(app: AppHandle, task_id: String) -> Result<(), String> {
    let state = app.state::<VideoState>();
...
#[tauri::command]
fn video_download_read(app: AppHandle, task_id: String) -> Result<Vec<u8>, String> {
    let state = app.state::<VideoState>();
...
#[tauri::command]
fn video_download_cleanup(app: AppHandle, task_id: String) -> Result<(), String> {
    let state = app.state::<VideoState>();
    let mut tasks = state.tasks.lock().unwrap();
